...
TEST_F(MatchingEngineSMPTest, SMP_None_AllowsSelfMatch) {
    ...
    auto* sell_order = createTestOrder(order_pool_, Side::SELL, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, 123);
    order_book_->addOrder(sell_order, 1000, 1);

    // Create buy order from same client
    auto* buy_order = createTestOrder(order_pool_, Side::BUY, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, 123);
...
TEST_F(MatchingEngineSMPTest, SMP_IgnoresWhenClientIdNegativeOne) {
    ...
    auto* sell_order = createTestOrder(order_pool_, Side::SELL, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, -1);
    order_book_->addOrder(sell_order, 1000, 1);

    // Create buy order also with client_id = -1
    auto* buy_order = createTestOrder(order_pool_, Side::BUY, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, -1);
...
TEST_F(MatchingEngineSMPTest, SMP_CancelResting_ContinuesMatching) {
    ...
    auto* sell1 = createTestOrder(order_pool_, Side::SELL, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, 123);
    order_book_->addOrder(sell1, 1000, 1);

    auto* sell2 = createTestOrder(order_pool_, Side::SELL, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, 456);
    order_book_->addOrder(sell2, 1001, 2);

    // Create buy order from client 123 with enough quantity to match both
    auto* buy_order = createTestOrder(order_pool_, Side::BUY, kPrice100, kQty20, TimeInForce::GOOD_TILL_CANCEL, 123);
...
TEST_F(MatchingEngineSMPTest, SMP_DifferentClients_NoSelfMatch) {
    ...
    auto* buy_order = createTestOrder(order_pool_, Side::BUY, kPrice100, kQty10, TimeInForce::GOOD_TILL_CANCEL, 456);
