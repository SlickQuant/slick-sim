...
([`hyperliquid_info_proxy.hpp`](https://github.com/SlickQuant/slick-sim/blob/main/src/venues/hyperliquid/hyperliquid_info_proxy.hpp)).
...
The subscription snapshot replays the instrument's recent trades, oldest first (empty if there are
none yet):
...
Live prints use the same shape, one message per executed trade.

`side` is the **aggressor's**: `"B"` for buy, `"A"` for sell. `hash` is always the literal `"0x0"` —
there is no chain. `tid` is a per-venue sequence generated by the simulator and stable across
clients: two clients see the same id for the same trade, live or replayed in a snapshot.
