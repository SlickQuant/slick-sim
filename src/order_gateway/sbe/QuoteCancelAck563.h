...
#ifndef _SBE_QUOTECANCELACK563_H_
#define _SBE_QUOTECANCELACK563_H_
...
#pragma clang diagnostic pop
    ...
    std::string getCancelledSymbolAsString() const
    {
    ...
    #if __cplusplus >= 201703L
    std::string_view getCancelledSymbolAsStringView() const SBE_NOEXCEPT
    {
        const char *buffer = m_buffer + m_offset + 312;
        size_t length = 0;
...
#pragma clang diagnostic pop
    ...
    std::uint64_t getOrigOrderUser(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 351, sizeof(char) * static_cast<size_t>(length));
        return length;
    }
...
#pragma clang diagnostic pop
    ...
    std::uint64_t getCancelText(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 359, sizeof(char) * static_cast<size_t>(length));
        return length;
    }
...
#pragma clang diagnostic pop
    ...
    SBE_NODISCARD BooleanNULL::Value quoteEntryOpen() const SBE_NOEXCEPT
    {
        if (m_actingVersion < 8)
        {
