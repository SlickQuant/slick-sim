...
#ifndef _SBE_ORDERMASSACTIONREPORT562_H_
#define _SBE_ORDERMASSACTIONREPORT562_H_
...
#pragma clang diagnostic pop
    ...
    std::uint64_t getOrigOrderUser(char *const dst, const std::uint64_t length) const
    {
        ...
        std::memcpy(dst, m_buffer + m_offset + 114, sizeof(char) * static_cast<size_t>(length));
        return length;
    }
...
#pragma clang diagnostic pop
    ...
    #pragma clang diagnostic pop
        ...
        std::string getOrigCIOrdIDAsString() const
        {
        ...
        #if __cplusplus >= 201703L
        std::string_view getOrigCIOrdIDAsStringView() const SBE_NOEXCEPT
        {
            const char *buffer = m_buffer + m_offset + 0;
            size_t length = 0;
