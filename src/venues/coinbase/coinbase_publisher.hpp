...
struct PerSocketData {
    ...
    std::array<SubscriptionInfo, (uint8_t)coinbase::WebSocketChannel::_CHANNEL_COUNT_> subscription_info;
};

class CoinbasePublisher : public WebsocketMarketDataPublisher {
    using wsT = uWS::WebSocket<false, true, PerSocketData>;

public:
    CoinbasePublisher(const json &config, slick::queue<Request> &request_queue, slick::queue<uint8_t> &market_data_queue);
    // void start() override;
    // void stop() override;
...
private:
    ...
    std::array<std::unordered_map<std::string, std::unordered_set<wsT*>>, (uint8_t)coinbase::WebSocketChannel::_CHANNEL_COUNT_> subscription_info_;
